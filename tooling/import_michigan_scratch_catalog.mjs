...
export function parseMichiganCatalog(body, day) {
  ...
  const games = listed.map(g => {
    ...
    if (!row || !Array.isArray(row.prizesRemainingData) || !row.prizesRemainingData.length) throw Error('Listed game missing prize inventory');
    ...
    return {stateName: 'Michigan', id: String(g.igtId), name: g.name.trim(), cost: money(g.displayedTicketPrice), topPrize,
  ...
  return {state:'Michigan',source,retrievedDate:day,sourceDate:null,
    coverage:'Retail instant games listed in the official web lobby and available in stores, matched by official IGT ID. Prize inventory is not dated claims or retailer-linked winning activity. Pull Tabs and unlisted inventory rows are excluded.',
...
async function main(output) {
  ...
  if(catalog.games.length<40)throw Error('Unexpectedly small Michigan retail instant catalog');
  ...
  console.log(`Validated ${catalog.games.length} Michigan games; ${catalog.excludedInventoryIds.length} unlisted inventory IDs excluded.`);
}
